...
export const ExpenseDebugPanel = ({ selectedGroupId }: { selectedGroupId?: string }) => {
  ...
  const refreshCacheData = useCallback(() => {
    ...
    const groupsData = queryClient.getQueryData(groupsKey) as { length: number }[] | undefined;
  ...
  const forceInvalidateAll = async () => {
    ...
    addLog('Force Invalidate Started', 'Starting aggressive cache invalidation');
    ...
    try {
      ...
      await queryClient.invalidateQueries({ 
        predicate: (query) => {
          ...
          return key.includes('allEntries') || 
      ...
      addLog('Force Invalidate Success', 'All queries invalidated and refetched');
    } catch (error) {
  ...
  useEffect(() => {
    refreshCacheData();
    const interval = setInterval(refreshCacheData, 2000); // Auto-refresh every 2s
        ...
        <AccordionDetails>
          <Box sx={{ p: 2 }}>
            ...
            <Box sx={{ display: 'flex', gap: 1, mb: 2, flexWrap: 'wrap' }}>
              <Button size="small" onClick={refreshCacheData} variant="outlined">
                Refresh
              </Button>
              <Button size="small" onClick={forceInvalidateAll} variant="contained" color="warning">
                Force Refetch
              </Button>
              <Button size="small" onClick={clearAllCache} variant="contained" color="error">
                Clear Cache
              </Button>
            </Box>
            ...
            <Box sx={{ fontSize: '0.75rem', mb: 2, color: colors.textSecondary }}>
              {Object.entries(cacheData).map(([key, data]) => (
                <Box key={key} sx={{ mb: 1 }}>
                  <strong>{key}:</strong> {data.hasData ? '✅' : '❌'} 
                  ({data.expenseCount || data.groupCount || data.categoryCount || 0} items)
                  <br />
                  <span style={{ fontSize: '0.7rem' }}>
                    Updated: {data.lastUpdated ? new Date(data.lastUpdated).toLocaleTimeString() : 'Never'}
                  </span>
                </Box>
              ))}
            </Box>
            ...
            <Box sx={{ fontSize: '0.7rem', mb: 2, color: colors.textSecondary }}>
              {Object.entries(cacheData).map(([key, data]) => (
            ...
            <Box sx={{ maxHeight: 200, overflow: 'auto', fontSize: '0.7rem' }}>
              {logs.map((log, i) => (
                <Box key={i} sx={{ mb: 1, p: 1, backgroundColor: colors.background, borderRadius: 1 }}>
                  <Box sx={{ color: colors.primary, fontWeight: 'bold' }}>
                    [{log.timestamp}] {log.event}
                  </Box>
                  <Box sx={{ color: colors.textSecondary, whiteSpace: 'pre-wrap' }}>
                    {log.data}
                  </Box>
                </Box>
              ))}
